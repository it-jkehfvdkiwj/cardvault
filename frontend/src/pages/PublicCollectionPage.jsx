  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 max-w-6xl mx-auto w-full px-6 py-4">
        {loading ? (
          <div className="flex justify-center py-24 text-ink-3"><Loader className="w-6 h-6 animate-spin" /></div>
        ) : error || !data ? (
          <div className="text-center py-24">
            <p className="text-5xl mb-4">🔒</p>
            <h1 className="text-xl font-bold">Sammlung nicht gefunden</h1>
            <p className="text-ink-3 text-sm mt-1">Dieser Link ist ungültig oder die Sammlung ist privat.</p>
          </div>
        ) : (
          <>
            ...
            {(() => {
              ...
              return visible.length === 0 ? (
                <p className="text-center text-ink-3 py-16">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                {visible.map((c) => (
                  <div key={c.id} className="panel p-2 flex flex-col gap-2">
                    <div className="relative aspect-[2.5/3.5] rounded-lg overflow-hidden bg-surface-3">
                      {c.image_url
                        ? <img src={c.image_url} alt={c.name} className="w-full h-full object-cover" loading="lazy" />
                        : <div className="w-full h-full flex items-center justify-center text-4xl text-ink-4">🃏</div>}
                      {c.quantity > 1 && (
                      ...
                      <div className="absolute bottom-1 left-1 flex gap-1">
                        {c.is_foil && <span className="bg-accent text-ink rounded p-0.5"><Sparkles className="w-3 h-3" /></span>}
                        {c.for_trade && <span className="bg-blue-500/90 text-ink rounded p-0.5"><ArrowLeftRight className="w-3 h-3" /></span>}
                      </div>
                    </div>
                    <div className="px-0.5 space-y-1">
                      <p className="text-xs font-semibold text-ink leading-tight line-clamp-2">{c.name}</p>
                      <p className="text-xs text-ink-3 truncate">{c.set_name}</p>
